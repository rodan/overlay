...
src_prepare() {
	...
	eapply "${FILESDIR}"/shorewall-core-5.2.8-egrep_fgrep_deprecation.patch
	popd &>/dev/null || die
	...
	if use ipv4; then
		...
		eapply "${FILESDIR}"/shorewall-5.2.8-egrep_fgrep_deprecation.patch
		popd &>/dev/null || die
	fi
	...
	if use ipv6; then
		...
		eapply "${FILESDIR}"/shorewall-5.2.1-no-gzipped-manpages.patch
		eapply "${FILESDIR}"/shorewall6-5.2.8-egrep_fgrep_deprecation.patch
		popd &>/dev/null || die
	fi
	...
	if use lite4; then
		...
		eapply "${FILESDIR}"/shorewall-lite-5.2.8-egrep_fgrep_deprecation.patch
		popd &>/dev/null || die
	fi
	...
	if use lite6; then
		...
		eapply "${FILESDIR}"/shorewall-lite-5.2.1-no-gzipped-manpages.patch
		eapply "${FILESDIR}"/shorewall6-lite-5.2.8-egrep_fgrep_deprecation.patch
		popd &>/dev/null || die
	fi
	...
	if use init; then
		...
		eapply "${FILESDIR}"/shorewall-init-5.2.8-egrep_fgrep_deprecation.patch
		popd &>/dev/null || die
	fi
